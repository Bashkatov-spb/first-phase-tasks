// Languages Statistic
// Реализуйте функцию getLanguagesStatistic, которая поможет IT журналу подвести итоги 2019 года по популярности использования 
// языков программирования. На входе функция получает массив отзывов пользователей. Необходимо вернуть объект в формате
...
const getLanguagesStatistic = (data) => {
  const data2019 = data.filter((el) => el.year === 2019);
  const arr = {};
  data2019.map((el) => {
    if (!arr[`${el.language}`]) {
      arr[`${el.language}`] = 0
